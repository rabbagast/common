...
{
  private final short imageWidth_;

  private final short imageHeight_;
    ...
    throws IOException
  {
    ...
    try {
      if (pixelGrabber.grabPixels() != true)
        throw new IOException("Unable to create GIF encoder: " + pixelGrabber.status());
    }
    catch (InterruptedException exception) {
      throw new IOException("Unable to create GIF encoder", exception);
    }
  ...
  /**
   * Writes the image to a stream in GIF format.
  ...
  private static void writeWord(OutputStream stream, short w)
    throws IOException
  {
    stream.write (w & 0xFF);
  ...
  private static void writeString(OutputStream stream, String string)
    throws IOException
  {
   ...
   * elements.
   */
  private final class LzwStringTable
  {
