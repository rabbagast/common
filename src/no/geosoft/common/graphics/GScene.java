...
{
  /** Window owner of this scene, Never null. */
  private final GWindow window_;

  /** World to device transformer for this scene. Never null. */
  private final GTransformer transformer_;

  /** Instance that does annotation placement for this scene. Never null. */
  private final GAnnotator annotator_;

  /** Viewport of this scene. Never null. */
  private final GViewport viewport_;

  /** Initial world extent. Never null. */
  private final GWorldExtent initialWorldExtent_;

  /** Current world extent. Never null. */
  private final GWorldExtent currentWorldExtent_;

  /**
   * Scroll handler. Initally null. If the client need scroll support
   * on the scene, install scroll handler with installScrollHandler().
   */
   ...
   *
   * @param window  Window to attach scene to. Non-null.
   * @param name    Name of this scene. Null if unnamed.
  ...
  public GScene(GWindow window, String name)
  {
    ...
    // TODO: Both can probably be removed
    shouldWorldExtentFitViewport_ = false;
    shouldZoomOnResize_ = false;  // "see bigger"
  ...
  /**
   * Create an unnamed scene within the specified window.
   *
   * @param window  Window to acttach this scene to. Non-null.
   * @throws IllegalArgumentException  If window is null.
   */
   ...
   * this object.
   *
   * @return  The scene of this GObject. Never null.
   */
  @Override
   ...
   *
   * @return  Window of this scene. Never null.
   */
  @Override
   ...
   *
   * @return  Current transformation object of this scene. Never null.
   */
   ...
   *
   * @param x0  X coordinate of viewport upper left corner.
   * @param y0  Y coordinate of viewport upper left corner.
   * @param x1  X coordinate of viewport upper right corner.
   * @param y1  Y coordinate of viewport upper right corner.
   * @param x2  X coordinate of viewport lower left.
   * @param y2  Y coordinate of viewport lower left.
   */
   ...
   * @param width   Width of viewport &lt;0,&gt;
  ...
  public void setViewport(int x0, int y0, int width, int height)
  {
    ...
    if (height < 0)
      throw new IllegalArgumentException("Invalid viewport width: " + height);
   ...
   *
   * @return  Current viewport of this scene. Never null.
   */
   ...
   * @throws IllegalArgumentException  If w0, w1 or w2 is null or not
   *                                   an array or 3.
   */
   ...
   *
   * @return  Current world extent. Never null.
   */
  ...
  private void adjustCurrentWorldExtent()
  {
    ...
    // Viewport dimensions
    double viewportWidth = viewport_.getWidth();
    double viewportHeight = viewport_.getHeight();
    ...
    currentWorldExtent_.extendWidth(newWorldWidth, 0.0);
    currentWorldExtent_.extendHeight(newWorldHeight, 1.0);
  }
  ...
  protected void resize(double dx, double dy)
  {
    ...
    viewport_.set(0, 0, window_.getWidth(), window_.getHeight());
  ...
  public void zoom(double[] w0, double[] w1, double[] w2)
  {
    ...
    // Make sure we keep aspect ratio (if requested)
    adjustCurrentWorldExtent();
  ...
  public void zoom(double x0, double y0, double width, double height)
  {
    double[] w0 = {x0,         y0,          0.0};
    double[] w1 = {x0 + width, y0,          0.0};
    double[] w2 = {x0,         y0 + height, 0.0};
   ...
   *
   * @param positionals  Positionals to compute positions of. Non-null.
   */
  void computePositions(Collection<? extends GPositional> positionals)
  {
    assert positionals != null : "positionals cannot be null";
   ...
   *
   * @param positional  Positional to compute position for. Non-null.
   */
  void computeVertexPositions(GPositional positional)
  {
    assert positional != null : "positional cannot be null";
   ...
   * @param vScrollBar  Vertical scrollbar (or null if a vertical scrollbar
   *                    is not used).
   */
