 ...
 */
public final class GImage extends GPositional
{
  ...
  {
    // Set back-end variables to null for lazy create
    imageData_ = null;
    image_ = null;
  }
  ...
  void computeSize()
  {
    ...
    // Otherwise the size is already correctly set.
      ...
      try {
        InputStream stream = new BufferedInputStream(new FileInputStream(file_.getPath()));
    ...
    if (image_ == null && imageData_ != null) {
      ...
      for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
          image.setRGB(i, j, imageData_[pointNo] == 0 ?
