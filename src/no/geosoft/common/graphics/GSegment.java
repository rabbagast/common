 ...
 */
public final class GSegment
  implements GStyleListener
{
  private final List<GText> texts_ = new ArrayList<GText>();
  private final List<GComponent> components_ = new ArrayList<GComponent>();
  private final List<GImage> images_ = new ArrayList<GImage>();
  ...
  /**
   * This method is called in a response to updated geometry,
   * new parent or new style settings.
   */
   ...
   * @param x  X coordinates.
   * @param y  Y coordinates.
   * @param z  Z coordinates. May be null if Z is to be ignored.
   */
  public void setGeometry(double x[], double y[], double z[])
  {
    if (owner_ == null || owner_.getScene() == null)
      return;
    ...
    for (int i = 0; i < x.length; i++) {
      ...
      int[] device = transformer.worldToDevice(world);
  ...
  {
    return Collections.unmodifiableList(texts_);
  }

  boolean hasText()
  {
    return texts_.size() > 0;
  }
  ...
  {
    return texts_.size() > 0 ? texts_.get(0) : null;
  }
  ...
  {
    // Update damage area
    if (owner_ != null && owner_.getWindow() != null) {
  ...
  {
    return Collections.unmodifiableList(components_);
  }
