...
import no.geosoft.common.geometry.Box;
  ...
  implements GStyleListener
{
  ...
  private final List<GObject> children_ = new ArrayList<GObject>();
  private final List<GSegment> segments_ = new ArrayList<GSegment>();
   ...
   *
   * @return  All segments of this object. Never null.
   */
  public List<GSegment> getSegments()
  {
    return Collections.unmodifiableList(segments_);
  }
  ...
  {
    return segments_.size() > 0 ? segments_.get(0) : null;
  ...
  {
    for (GSegment segment : segments_) {
  ...
  {
    List<GSegment> segments = new ArrayList<GSegment>();
  ...
  public GSegment findSegment(int x0, int y0, int x1, int y1)
  {
    ...
    return segments.size() == 0 ? null : segments.get(segments.size() - 1);
  }
   ...
   *
   * @param x  X coordinate of point to check.
   * @param y  Y corrdinate of point to check.
   ...
   * @param segments  List to add segments to.
   */
  private void findSegments (int x, int y,
                             List<GSegment> segments)
  {
    // Don't scan any furher if the region doesn't intersect
  ...
  public GObject find(int x0, int y0, int x1, int y1)
  {
    ...
    return objects.size() == 0 ? null : objects.get(objects.size() - 1);
  }
  ...
  {
    List<GObject> objects = new ArrayList<GObject>();
   ...
   * @param objects  Object collection to add to.
   */
  private void findAllInside(int x0, int y0, int x1, int y1,
  ...
  public void removeAll()
  {
    ...
    Collection<GObject> children = new ArrayList<GObject>(children_);
  ...
  public GSegment getSegment(int segmentNo)
  {
    ...
    return segmentNo < 0 || segmentNo >= nSegments ? null : segments_.get(segmentNo);
  }
   ...
   *
   * @param from  Starting segment
   * @param to    Ending segment (-1 indicates to end).
   */
  public void removeSegments(int from, int to)
  {
    if (from >= segments_.size())
      return;
  ...
  void computeTextPositions()
  {
    ...
    for (GSegment segment : segments_) {
      scene.computePositions(segment.getTexts());
    }
  }
  ...
  void computeComponentPositions()
  {
    ...
    // Do annotation on all segments
    for (GSegment segment : segments_) {
      scene.computePositions(segment.getComponents());
    }
  }
  ...
  void computeImagePositions()
  {
    ...
    for (GSegment segment : segments_) {
      scene.computePositions(segment.getImages());
