  ...
  implements Cloneable
{
  ...
  private final Collection<WeakReference<GStyleListener>> listeners_ = new ArrayList<WeakReference<GStyleListener>>();
  ...
  private double         gradientX0_;
  private double         gradientY0_;
  private Color          gradientColor1_;
  private double         gradientX1_;
  private double         gradientY1_;
  ...
  public Object clone()
  {
    ...
    style.gradientX0_     = gradientX0_;
    style.gradientY0_     = gradientY0_;
    style.gradientColor1_ = gradientColor1_;
    style.gradientX1_     = gradientX1_;
    style.gradientY1_     = gradientY1_;
  ...
  void update(GStyle style)
  {
    ...
    if (style.isValid (MASK_GRADIENT))
      setGradient(style.gradientX0_, style.gradientY0_, style.gradientColor1_,
                  style.gradientX1_, style.gradientY1_, style.gradientColor2_);
  }
  ...
  public void setGradient(double x0, double y0, Color color1,
                          double x1, double y1, Color color2)
  {
    gradientX0_ = x0;
    gradientY0_ = y0;
  ...
  public void setFillPattern(int fillType)
  {
    ...
    switch (fillType) {
      ...
      default :
        assert false : "Unexpected fill type: " + fillType;
    }
  ...
  public void setLineStyle(int lineStyle)
  {
    ...
    switch (lineStyle) {
      ...
      default :
        assert false : "Unexpected line style: "+ lineStyle;
    }
  ...
  Paint getPaint(Rect rectangle)
  {
    ...
    // Gradient background fill
      ...
      double x0 = rectangle.x + rectangle.width * gradientX0_;
      double y0 = rectangle.y + rectangle.height * gradientY0_;
      double x1 = rectangle.x + rectangle.width * gradientX1_;
      double y1 = rectangle.y + rectangle.height * gradientY1_;

      paint_ = new GradientPaint((float) x0, (float) y0, gradientColor1_,
                                 (float) x1, (float) y1, gradientColor2_,
                                 false);
    }
  ...
  {
    for (Iterator<WeakReference<GStyleListener>> i = listeners_.iterator(); i.hasNext(); ) {
      WeakReference<GStyleListener> reference = i.next();
      GStyleListener listener = reference.get();
  ...
  void addListener(GStyleListener listener)
  {
    ...
    for (WeakReference<GStyleListener> reference : listeners_) {
      if (reference.get() == listener)
        return;
    }

    // Add the listener
    listeners_.add(new WeakReference<GStyleListener>(listener));
  }
